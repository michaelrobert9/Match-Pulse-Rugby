  ...
  for (const inviteDoc of snap.docs) {
    ...
    try {
      if (invite.orgId) {
        ...
        // run before the sign-in bootstrap has created this user's rugby-local
        // doc, and update() throws on a missing doc. A merge-set creates it if
        // absent and deep-merges the orgRoles map (other memberships preserved)
        // if present — and the create/self-update rules both permit it. A
        // set+update pair in one batch would fail: rules evaluate the update
        // against the pre-batch (absent) doc.
