...
// source appears in, its scoring events, and its stat slices repoint to the
// target, org links are unioned onto the target, and the source is tombstoned
...
export async function mergePeople(sourceId, targetId) {
  ...
  // 1. Repoint every match the source appears in. Rugby scoring events live in
  //    `m.scores`, keyed by scorerPersonId (no assist concept).
  for (const m of await matchesWithPerson(sourceId)) {
    ...
    if (Array.isArray(m.scores) && m.scores.some(s => s.scorerPersonId === sourceId)) {
      patch.scores = m.scores.map(s => s.scorerPersonId === sourceId
        ? { ...s, scorerPersonId: targetId, scorerName: tName ?? s.scorerName }
        : s)
    }
  ...
  //    for the same team+competition+season (avoids a duplicate slice). Rugby
  //    slice tallies: caps/tries/conversions/penalties/dropGoals/points + cards.
  ...
  for (const d of srcSlices.docs) {
    ...
    if (dup) {
      await updateDoc(doc(db, 'players', dup.id), {
        ...
        tries: (dup.tries ?? 0) + (s.tries ?? 0),
        conversions: (dup.conversions ?? 0) + (s.conversions ?? 0),
        penalties: (dup.penalties ?? 0) + (s.penalties ?? 0),
        dropGoals: (dup.dropGoals ?? 0) + (s.dropGoals ?? 0),
        points: (dup.points ?? 0) + (s.points ?? 0),
        cards: {
...
export async function updateTeam(id, data) {
  ...
  if (structuralChange) {
    ...
    // team to being unsearchable by its school/club name. orgName and the
    // per-team name are stripped/absent here, so recover them from the stored doc.
    let on = orgName
    let existingTeamName = 'teamName' in patch ? patch.teamName : undefined
    if (!on || existingTeamName === undefined) {
      const cur = await getDoc(doc(db, 'teams', id)).catch(() => null)
      const cd  = cur && cur.exists() ? cur.data() : {}
      if (!on) on = cd.orgName ?? null
      if (existingTeamName === undefined) existingTeamName = cd.teamName ?? null
    }
    if (name) {
      patch.displayName = name
      patch.searchName  = [on, name, existingTeamName].filter(Boolean).join(' ').toLowerCase()
    } else if (existingTeamName) {
      // Association/league team identified only by its own name — there is no
      // structured label, so clear the stored label rather than leaving a stale one.
      patch.displayName = ''
...
export async function resetMatch(id) {
  ...
  if (m.competitionId) {
    ...
    // reset would otherwise leave this match's scores/cards/caps stuck on the
...
export async function seedFixturesFromTeamSheet(competitionId, teamId, squad = []) {
  ...
  // Maintain lineupPersonIds — the flat reverse-index that powers a player's
  // match list, the merge tool, and the stats self-heal — WITHOUT populating the
  // real homeLineup/awayLineup. The team-sheet renderer/editor treats a side as
  // "derived" (and editable via the paste-and-review grid) only while its lineup
  // array is empty, so writing real entries here would disable that editor. We
  // keep the array empty (reverting any earlier seed that wrongly populated it)
  // and just index the personIds. Frozen (played) fixtures are never touched.
...
// POM matches are read from `playerOfMatch`; the two are never migrated so no
// historical award is lost. See src/lib/pom.js#pomForSide.
...
// ── Billing / entitlement ─────────────────────────────────────────────────────
// Intentionally absent. Plan purchase, payment requests, manual grants and
// activation codes are CENTRAL and owned by the main site (matchpulse.co.za):
// it runs the PayFast checkout, its ITN webhook grants the plan, and
// syncUserClaims mirrors the result onto the user's Auth token as custom
// claims. This app only READS that claim (see contexts/AuthContext.jsx and
// lib/entitlement.js). Client writes to entitlement / eventCredits /
// entitlementExpiresAt are rejected by Firestore rules.
