...
import { matchTries } from './rugbyScoring.js'
...
//   stats           — count timeline scoring events/cards toward stats?
//   home, away      — the points standings should read
//   homeTries,      — try counts for bonus-point calculation; null when the
//   awayTries         count is unknown (awarded results never carry tries)
//   countsAllEvents — count EVERY timeline event (incl. any abandoned-attempt
//                     scores); false means exclude events flagged abandonedAttempt.
export function fixtureContribution(m) {
  const o = m && typeof m === 'object' ? m.outcome : null

  if (o && o.kind) {
    if (o.kind === OUTCOME_KIND.NOT_PLAYED) {
      return { standings: false, stats: false, home: 0, away: 0, homeTries: null, awayTries: null, countsAllEvents: false }
    }
    if (o.flag === RESULT_FLAG.AWARDED) {
      // Awarded score sits in the normal slot; stats ignore it. No real play →
      // no tries, so an awarded result can never earn a try bonus point.
      return { standings: true, stats: false, home: m.homeScore ?? 0, away: m.awayScore ?? 0, homeTries: null, awayTries: null, countsAllEvents: false, awarded: true }
    }
    if (o.flag === RESULT_FLAG.FROZEN) {
      return { standings: false, stats: false, home: 0, away: 0, homeTries: null, awayTries: null, countsAllEvents: false }
    }
    if (o.flag === RESULT_FLAG.FINAL) {
      // Abandoned-stands: the frozen attempt IS the result — count its events too.
      const stands = o.kind === OUTCOME_KIND.ABANDONED
      return {
        standings: true, stats: true, home: m.homeScore ?? 0, away: m.awayScore ?? 0,
        homeTries: matchTries(m, 'home'), awayTries: matchTries(m, 'away'),
        countsAllEvents: stands,
      }
    }
  }
  ...
  return {
    standings: isFinal, stats: isFinal,
    home: m?.homeScore ?? 0, away: m?.awayScore ?? 0,
    homeTries: isFinal && typeof m === 'object' ? matchTries(m, 'home') : null,
    awayTries: isFinal && typeof m === 'object' ? matchTries(m, 'away') : null,
    countsAllEvents: false,
  }
}
...
// The competition's walkover score, with the sides normalised. Default 28–0 to
// the opponent (the World Rugby standard walkover scoreline; configurable per
// competition — existing rule shape: { concedingTeam, opposingTeam }).
export function walkoverScore(competition) {
  const w = competition?.rules?.walkoverScore ?? { concedingTeam: 0, opposingTeam: 28 }
  return { conceding: Number(w.concedingTeam ?? 0), opposing: Number(w.opposingTeam ?? 28) }
}
