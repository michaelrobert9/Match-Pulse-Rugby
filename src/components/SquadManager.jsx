                  ...
                  {confirmId === p.id ? (
                    <span className="flex items-center gap-1.5 shrink-0">
                      <button onClick={() => remove(p)} disabled={busy}
                        title={((p.caps ?? 0) > 0 || (p.points ?? 0) > 0 || (p.tries ?? 0) > 0) ? 'This entry has recorded stats — removing it deletes them.' : 'Remove from squad'}
                        className="text-[10px] font-bold uppercase tracking-widest text-red-600 hover:text-red-500 disabled:opacity-40">
                        {busy ? '…' : (((p.caps ?? 0) > 0 || (p.points ?? 0) > 0 || (p.tries ?? 0) > 0) ? 'Remove + stats' : 'Remove')}
                      </button>
