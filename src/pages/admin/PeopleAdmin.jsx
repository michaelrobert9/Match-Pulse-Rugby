...
// All of the duplicate's matches, scoring events and stat slices repoint to this
// profile, its org links are unioned in, and the duplicate is retired. Shown
// with a preview of what will move before it commits.
function MergeSection({ person }) {
  ...
  async function confirmMerge() {
    if (!dup) return
    if (!confirm(`Merge "${dup.fullName}" into "${person.fullName}"? The duplicate's matches, scores and stats move to ${person.fullName}, and "${dup.fullName}" is retired. This can't be auto-undone.`)) return
    setBusy(true); setErr('')
  ...
  return (
    <div className="px-4 py-4 border-t border-slate-200">
      ...
      <p className="text-[12px] text-slate-500 mb-3">If the system made a second record for {person.fullName}, fold it in here — its matches, scores and stats all move onto this profile.</p>
      {dup ? (
