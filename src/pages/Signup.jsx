...
export default function Signup() {
  ...
  async function handleStep1(e) {
    ...
    try {
      ...
      // One person, one MatchPulse account. If it already exists, don't fork a
      // second one — send them to sign in instead.
  ...
  async function saveProfile() {
    ...
    // CENTRAL identity → the shared (default) database. Only identity fields —
    // never sport data, and never plan/billing fields (those are the main
    // site's). The users/{uid} shell itself is created by AuthContext on
    // sign-in; here we just publish the public profile subset.
    ...
    // SPORT profile → rugby's OWN database (rugbyProfiles/{uid}), keyed by the
    // central UID. Position/role/etc. must never land in the shared users doc.
    await saveRugbyProfile(createdUid, {
      role,
  ...
  async function handleStep2(e) {
    ...
    try {
      ...
      // The unclaimed-profile claim search runs on the session (see
      // ClaimPrompt), not here, so it also fires for Google sign-ins.
      navigate('/portal')
    } catch (err) {
